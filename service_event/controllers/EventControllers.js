...
exports.ajouterEvent = (req, res) => {
    const { titre, description, prix, lieu, places_disponibles, date_deb, date_fin, type_id, categorie_id, photo_url } = req.body;
    if (!titre || !description || !prix || !lieu || !places_disponibles || !date_deb || !date_fin || !type_id || !categorie_id || !photo_url) {
        return res.status(400).json({ message: 'Tous les champs sont requis.' });
    }

    db.query('INSERT INTO evenement (titre, description, prix, lieu, places_disponibles, date_deb, date_fin, type_id, categorie_id, photo_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
        [titre, description, prix, lieu, places_disponibles, date_deb, date_fin, type_id, categorie_id, photo_url], (err, result) => {
        if (err) {
            console.error(err);
...
exports.modifierEvent = (req, res) => {
    const { titre, description, prix, lieu, places_disponibles, photo_url, date_deb, date_fin, categorie_id, type_id } = req.body;
    const id = req.params.id;

    if (!titre || !description || !prix || !lieu || !places_disponibles || !photo_url || !date_deb || !date_fin || !categorie_id || !type_id) {
        return res.status(400).json({ message: 'Tous les champs sont requis pour la modification.' });
    }

    db.query('UPDATE evenement SET titre = ?, description = ?, prix = ?, lieu = ?, places_disponibles = ?, photo_url = ?, date_deb = ?, date_fin = ?, categorie_id = ?, type_id = ? WHERE id = ?', 
        [titre, description, prix, lieu, places_disponibles, photo_url, date_deb, date_fin, categorie_id, type_id, id], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ message: 'Erreur interne du serveur.' });
        }
...
exports.ajouterCategorie = (req, res) => {
    const { nom, description, type_id } = req.body;
    if (!nom || !description || !type_id) {
        return res.status(400).json({ message: 'Le nom de la catégorie est requis.' });
    }

    db.query('INSERT INTO categorie (nom, description, type_id) VALUES (?, ?, ?)', [nom, description, type_id], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ message: 'Erreur interne du serveur.' });
        }
...
exports.modifierCategorie = (req, res) => {
    ...
    db.query('UPDATE categorie SET nom = ?, description = ?, type_id = ? WHERE id = ?', [nom,description, type_id, id], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ message: 'Erreur interne du serveur.' });
        }
